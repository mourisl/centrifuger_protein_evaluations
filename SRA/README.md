## Procedures to evaluate WGS samples downloaded from SRA (species-in, species-not-in)
...
```
centrifuger-inspect --conversion-table -x cfr_pv > cfr_pv_nodes.dmp
```

4. Extract samples
```
perl SelectSra.pl Bacteria_SraRunInfo_shuf.csv nodes.dmp cfr_pv_nodes.dmp 100 1 > speciesin_sra.list
perl SelectSra.pl Bacteria_SraRunInfo_shuf.csv nodes.dmp cfr_pv_nodes.dmp 100 0 > speciesnotin_sra.list
```
